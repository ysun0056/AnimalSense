...
//Database query 
$rows = $wpdb->get_results("SELECT x.VernacularName, x.Latitude, x.Longitude,x.Address, ( 3959 * acos( cos( radians($center_lat) ) * cos( radians( x.Latitude ) ) * cos( radians( x.Longitude ) - radians($center_lng) ) + sin( radians($center_lat) ) * sin( radians( x.Latitude ) ) ) ) AS distance, x.Year, y.image as image FROM allanimals x JOIN animal_images y ON x.VernacularName = y.AnimalName HAVING distance < $radius ORDER BY distance LIMIT 0 , 20;");
