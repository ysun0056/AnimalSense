    ...
    <script>
       ...
       function setMap(){ //set the default map
        ...
        function getLocation() { //get the animal location
          ...
          var searchUrl = '/queryData?ID='+number; 
          downloadUrl(searchUrl, function(data) { 
          var xml = parseXml(data); 
          var markerNodes = xml.documentElement.getElementsByTagName("marker");
              if(markerNodes.length < 1)
                  {
                      alert("No animal information found");
                  }
            for (var i = 0; i < markerNodes.length; i++) {
                ...
                //var count = parseInt(markerNodes[i].getAttribute("count"));
                var address = markerNodes[i].getAttribute("add");
                //state = document.getElementById("animalState");
                var marker = L.marker([lat,lng]).addTo(map);
                
                var html = '<div id="content">'+
		      '<div><p><b>Last Seen:</b>'+year+'<br><b>Latitude:</b>'+lat+'<br><b>Longitude:</b>'+lng+'<br><b>Location:</b>'+address+'</p></div>'+
		      '</div>';
